...
interface CarouselContextType {
  ...
  scrollSnaps: unknown
  onDotButtonClick: unknown
  scrollProgress: unknown
  selectedSnap: unknown
  snapCount: unknown
  isScale: boolean
  slidesrArr: ThumbnailSlide[]
  setSlidesArr: unknown
  emblaThumbsRef: unknown
  onThumbClick: unknown
  carouselId: string
}
...
const Carousel: React.FC<CarouselProps> = ({
  ...
  return (
    ...
    >
      <div
        className={cn(className, 'overflow-hidden mx-auto w-full h-full  ')}
        ref={emblaRef}
      >
        {children}
      </div>
...
export const SliderContainer = ({
  ...
  return (
    <div
      className={cn(
        'flex   rounded-md shadow-[11px_11px_39px_#949494,-11px_-11px_39px_#ffffff] relative w-[80vw] aspect-square m-11 rounded-full bg-[#e0e0e0]  overflow-hidden',
        className
      )}
      style={{
...
export const Slider: React.FC<SliderProps> = ({
  ...
  //             boxShadow: '11px 11px 22px #a4a4a4, -11px -11px 22px #ffffff',
  //           }}
  //           className="relative h-[500]  w-[500] aspect-square  rounded-full bg-[#e0e0e0]  overflow-hidden"
  //         >
  //           {children}
  //         </figure>
  return (
...
export const SliderPrevButton = ({
  ...
  const { onPrevButtonClick, prevBtnDisabled }: unknown = useCarouselContext()
  return (
    <button
...
export const SliderNextButton = ({
  ...
  const { onNextButtonClick, nextBtnDisabled }: unknown = useCarouselContext()
  return (
    <>
      <button
        className={cn('', className)}
...
export const SliderProgress = ({ className }: { className?: string }) => {
  const { scrollProgress }: unknown = useCarouselContext()
  return (
    <div
      className={cn(
...
export const SliderDotButton = ({
  ...
  const { selectedIndex, scrollSnaps, onDotButtonClick, carouselId }: unknown =
    useCarouselContext()
  return (
    <div className={cn('flex', className)}>
      <div className="flex gap-2">
        {scrollSnaps.map((_: unknown, index: React.Key | null | undefined) => (
          <button
            type="button"
            key={index}
